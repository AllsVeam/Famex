...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['id'])) {
        ...
        if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] === UPLOAD_ERR_OK) {
            ...
            $consulta = "UPDATE espectaculos SET idFamex = '$idFamex', descripcion = '$descripcion', imagen = '$imagen' WHERE id = $id ";
        } else {
            $consulta = "UPDATE espectaculos SET idFamex = '$idFamex', descripcion = '$descripcion' WHERE id = $id ";
        }        
        $resultado = mysqli_query($conexion, $consulta);
        if ($resultado) {
            echo '<script type="text/javascript">
                window.location.href="../formEspectaculos.php?actualizar=1";
        ...
        if (isset($_POST['descripcion']) && is_array($_POST['descripcion'])) {
            ...
            if (isset($_FILES['imagen']) && is_array($_FILES['imagen']['tmp_name']) && count($_FILES['imagen']['tmp_name']) === count($descripciones)) {
                ...
                for ($i = 0; $i < $numElementos; $i++) {
                    ...
                    $consulta = "INSERT INTO espectaculos (descripcion, imagen, idFamex) VALUES ('$descripcion', '$imagen', '$idFamex')";
                    $resultado = mysqli_query($conexion, $consulta);
                    
                    // Verificar el resultado de la consulta
                    if ($resultado) {
                        echo '<script type="text/javascript">
                            window.location.href="../formEspectaculos.php?agregar=1";
                            mysqli_close($conexion);
                            </script>';
                    } else {
