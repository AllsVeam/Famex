...
<head>
    ...
    <title>Organizaciones | Feria Aeroespacial México</title>
    <!-- Favicon -->
...
<body>
    ...
    <div class="container">
        <a href="formOrg-agregar.php?id=<?php echo $idOrg ?>" style="float: right; padding: 15px;" class="icon-plus"
            title="Agregar"></a>
    </div>
    <div class="container tablas">
        <table class="table table-hover" style="border-radius: 15px;">
            ...
            <tbody>
                ...
                $sql = "SELECT * FROM organizaciones WHERE idTipoOrga = $idOrg LIMIT $offset, $registrosPorPagina";
                $result = mysqli_query($conexion, $sql);

                // Obtener el total de registros
                $sqlTotal = "SELECT COUNT(*) AS total FROM organizaciones WHERE idTipoOrga = $idOrg";
                ...
                while ($mostrar = mysqli_fetch_array($result)) {
                    ...
                    <tr>
                        ...
                        <td style="text-align: center;">
                            <a href="formOrg-agregar.php?idOrg=<?php echo $mostrar['id'] ?>&id=<?php echo $idOrg ?>"
                                class="icon-loop2" title="Actualizar"></a>&nbsp;&nbsp;&nbsp;
                            <a href="#" class="icon-bin" title="Borrar"
                            onclick="modalEliminar2('¿Estas seguro que desea borrar el registro?', '<?php echo $mostrar['id'] ?>', '<?php echo $mostrar['idTipoOrga'] ?>' )">
                            </a>
                        </td>
                    </tr>
                    <?php
                ...
                <ul class="pagination justify-content-center">
                    <?php if ($paginaActual > 1): ?>
                        <li class="page-item"><a class="page-link"
                                href="?pagina=<?php echo ($paginaActual - 1); ?>&id=<?php echo $idOrg ?>"><</a>
                        </li>
                    <?php endif; ?>
                    ...
                    <?php for ($i = 1; $i <= $totalPaginas; $i++): ?>
                        <li <?php if ($paginaActual == $i)
                            echo ' class="page-item active"'; ?>><a class="page-link"
                                href="?pagina=<?php echo $i; ?>&id=<?php echo $idOrg ?>"><?php echo $i; ?></a></li>
                    <?php endfor; ?>

                    <?php if ($paginaActual < ($totalRegistros / $registrosPorPagina)): ?>
                        <li class="page-item"><a class="page-link"
                                href="?pagina=<?php echo ($paginaActual + 1); ?>&id=<?php echo $idOrg ?>">></a></li>
                    <?php endif; ?>
                </ul>
            <?php endif; ?>
        </div>
    ...
    <script type="text/javascript">
        function preguntar(id, idSer) {            
                window.location.href = "php/orga-Del.php?id=" + id + "&idSer=" + idSer;            
        }
    </script>
</body>
